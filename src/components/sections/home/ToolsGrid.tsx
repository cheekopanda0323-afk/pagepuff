            ...
            <button
              ...
              className={`rounded-full border px-4 py-2 text-xs font-semibold whitespace-nowrap backdrop-blur-xl transition-all duration-300 sm:px-5 sm:py-2.5 sm:text-sm ${
                activeCategory === cat.id
                  ...
                  : "border-gray-200/80 bg-white/60 text-gray-600 hover:border-gray-300 hover:bg-white"
              }`}
            >
              {cat.label}
            </button>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredTools.map((tool) => (
            <Link
              key={tool.href}
              href={tool.href}
              className="group relative flex min-h-[200px] flex-col overflow-hidden rounded-3xl border border-white/60 bg-white/70 p-6 shadow-sm backdrop-blur-xl transition-all duration-500 hover:-translate-y-1.5 hover:border-white hover:shadow-2xl hover:shadow-black/5"
            >
              <div
